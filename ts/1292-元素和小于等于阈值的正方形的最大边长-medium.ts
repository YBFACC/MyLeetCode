...
function init(RowMax: number, ColMax: number, mat: number[][]): number[][] {
  const temp = Array.from({ length: RowMax + 1 }, () => Array.from({ length: ColMax + 1 }, () => 0))
  temp[1][1] = mat[0][0]

  for (let i = 1; i <= RowMax; i++) {
    for (let j = 1; j <= ColMax; j++) {
      temp[i][j] = temp[i - 1][j] + temp[i][j - 1] + mat[i - 1][j - 1] - temp[i - 1][j - 1]
    }
  }
  return temp
}

function helper(prefix: number[][], x0: number, y0: number, x1: number, y1: number): number {
